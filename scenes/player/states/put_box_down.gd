...
func state_entered() -> void:
	...
	player.animation_tree.animation_finished.connect(_on_put_down_1_finished)
	player.animation_tree.set(
...
# On box held
func _on_put_down_1_finished(anim_name: String) -> void:
	if anim_name != "PutBoxDown1": return
	
	# Disconnect callable
	player.animation_tree.animation_finished.disconnect(_on_put_down_1_finished)
	...
	player.animation_tree.animation_finished.connect(_on_put_down_2_finished)


func _on_put_down_2_finished(anim_name: String) -> void:
	if anim_name != "PutBoxDown2": return
	
	# Disconnect callable
	player.held_object = null
	player.animation_tree.animation_finished.disconnect(_on_put_down_2_finished)
